...
class Kilpailu(Frame):
    ...
    def _update(self):
        self.timestr.set(time.strftime("%H:%M:%S",time.gmtime(self.getKisaaika()))+(',%02d'%int(((self.getKisaaika()-int(self.getKisaaika()))*100))))
            ...
            if len(self.syotanumero.get()) > 0:
                ...
                for obj in self.competitors:
                    ...
                    """elif obj.bibnumber != self.syotanumero.get():
                        ...
                        break"""
                if self.found == 0:
                    self.kilpatemp = kilpailija('Etunimi','Sukunimi','-','-','Tarkistettavat:',self.syotanumero.get())
                    self.__kilpailijoita = self.__kilpailijoita + 1
                    for luokka in self.luokat:
                        ...
                        if obj.dnf == False:
                            if obj.GetTimeAmount() > 1:
                                if(position == 1):
                                    ...
                                    r.td(self.ConvertTimeToString(obj.totaltime)+ " +" + self.ConvertTimeToString(obj.totaltime - nr1pos.totaltime))
                                for aika in obj.valiajat:
...
#         f.write(fpart + script + str(h) + lpart)
    ...
    def writetotxt(self):
        f = open(self.__competiontName+'_' + time.strftime("%d_%m_%y_%H_%M_%S",time.localtime(time.time())) + '.txt', 'a')
    ...
    def writeOfficialTimes(self,tyyppi,query):
        if query == False:
            f = open("results/"+str(self.__competiontName)+'results_' + time.strftime("%d_%m_%y_%H_%M_%S",time.localtime(time.time())) + '.txt', 'a')
        if query == True:
            f = asksaveasfilename()
        #contents = str('Tilasto_' + time.strftime("%d_%m_%y_%H_%M_%S",time.localtime(time.time())) + '.txt')

        contents = str(self.__competiontName)+str(' ' + time.strftime("%y_%m_%d %H.%M.%S",time.localtime(time.time()))+'\n\n')
        self.temporary2 = sorted(self.competitors, key=attrgetter('totaltime'))
        self.temporary = sorted(self.temporary2, key=attrgetter('valiaikamaara'),reverse=True)
